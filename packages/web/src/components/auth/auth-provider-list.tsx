...
export const AuthProviderLayoutContext = React.createContext(false)

const COMPACT_FROM = 4

export interface AuthProviderListProps extends React.HTMLAttributes<HTMLDivElement> {
 ...
 */
const AuthProviderList = React.forwardRef<HTMLDivElement, AuthProviderListProps>(
      ...
      <div ref={ref} className={cn("mt-6", className)} {...props}>
        {label ? (
          <div
            ...
            style={{ color: "var(--auth-subtle,#98A1AE)" }}
          >
            <span className="h-px flex-1" style={{ background: "var(--auth-input-border,rgba(15,23,41,.14))" }} />
            {label}
            <span className="h-px flex-1" style={{ background: "var(--auth-input-border,rgba(15,23,41,.14))" }} />
          </div>
        ) : null}
        <AuthProviderLayoutContext.Provider value={compact}>
          <div className={cn("flex gap-3", compact ? "flex-wrap justify-center" : "flex-col")}>{enabled}</div>
        </AuthProviderLayoutContext.Provider>
      </div>
    )
  }
)
AuthProviderList.displayName = "AuthProviderList"
