...
- ac36bd5: Support lucide-react 1.x.
...
- fa0300c: AlertDialog: portal the overlay to `document.body`, and use the destructive colour for destructive confirms.
...
- 41be1f7: ConfirmDialog can describe its confirm button, and destructive confirmations no longer dismiss on outside click
...
- d2a6034: AuditLogViewer: per-entry expand/collapse, composable parts, and accessibility fixes.
...
- 00fa83c: Paint the auth surface's primary button, its shadow and the card border from the design tokens when no `brandColor` is given.
...
- 8e29b30: Make `brandColor` optional on `AuthPanel` and `AuthBackground`.
